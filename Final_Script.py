...
print("output: ", trans_ids['ENSMUST00000027263'][max(trans_ids['ENSMUST00000027263'].keys())])
...
print("of targeted exons not present in Ensembl canonical file: ", len(non_canonical_exons))
...
for i in exon_locations:
    if (type(i) == list):
        ...
        gene_phases[i[0]] = [exon_dict[i[0]][1],exon_dict[i[1]][-1]]
    else:
        if (not check(i)): continue
        gene_phases[i] = [exon_dict[i][1],exon_dict[i][-1]]
